...
routes.post("/register", upload.single("avatar"), async (req, res) => {
  const { cin, nom, prenom, role, Adr, Num_tel, MPass } = req.body;
  ...
  try {
    if (role == "client") {
      ...
      if (req.file) {
        newStation.avatar =
          "http://192.168.43.230:3001/uploads/" + req.file.filename;
        /* req.files.map(file => {
...
/*routes.post("/register-mobile", async (req, res) => {
  ...
  const form = new formidable.IncomingForm();
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      console.log(fields)
      const { cin, nom, prenom, role, Adr, Num_tel, MPass } = fields
      ...
      try {
        if (role == "client") {
          ...
          if (files.avatar) {
            let filename = saveFile(files.avatar);
            newStation.avatar =
              "http://192.168.43.230:3001/uploads/" + filename;
            /* req.files.map(file => {
...
*/
routes.post("/upload-avatar", upload.single("avatar"), (req, res) => {
  res.send("http://192.168.43.230:3001/uploads/" + req.file.filename);
})
...
routes.get("/getAll", async (req, res) => {
  try {
    const st = await Station.find({}).populate({
      path: "reservation",
      populate: {
        path: "Utilistauer",
      },
...
routes.post("/loginST", async (req, res) => {
  await Station.findOne({ email: req.body.email })
    .exec()
    .then((station) => {
      if (!station) {
        ...
        async (err, result) => {
          ...
          if (result) {
            ...
            await Token.findOneAndUpdate(
              { _stationId: station._id, tokenType: "login" },
              { token: token },
...
//get station by id
routes.get("/getStationbyId/:id", async (req, res) => {
  try {
    const oneStation = await Station.findOne({ _id: req.params.id });
...
routes.post("/login", (req, res) => {
  Utilisateur.findOne({ Adr: req.body.Adr })
    .exec()
    .then((utilisateur) => {
      if (!utilisateur) {
        return res.status(401).json({
...
routes.get("/logoutstation", auth, (req, res) => {
  Token.findOneAndDelete(
    { _stationId: req.stationId, type: "login" },
    (err, doc) => {
      if (err)
        return res.status(401).json({
          status: false,
...
routes.put("/ms/:id",upload.single("avatar"), async (req, res) => {
  try {
...
routes.get("/getutilisateurs", async (req, res) => {
  try {
    role = "client";
    const list = await Utilisateur.find({ role }).populate({
      path: "reservation",
      populate: {
        path: "Station",
      },
